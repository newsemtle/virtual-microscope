    ...
    async getAllMeasurements(imageIdentifier) {
        ...
        for (let i = 0; i < result.length; i++) {
            let measurement = new Measurement(
                ...
                result[i].name, result[i].color, this.plugin.conversionFactor, this.plugin.units, this.plugin.fabricCanvas, this.imageIdentifier, this.plugin,
            );
...
class OSDMeasure {
    ...
    addMeasurement(event) {
        ...
        if (this.p1) {
            this.p2 = new Point(imagePoint.x, imagePoint.y, this.measurementColor, this.fabricCanvas, this.imageIdentifier, this.viewer);
            this.p2.render(zoom);

            const measurement = new Measurement(
                this.p1, this.p2,
                `M${this.measurements.length + 1}`,
                this.measurementColor, this.conversionFactor, this.units, this.fabricCanvas, this.imageIdentifier, this,
            );
            ...
            this.setMeasureMode(false); // ✅ 측정 끝나면 자동 비활성화
            this.saveInLocalStorage(this.imageIdentifier);
            document.dispatchEvent(new Event("measurement-added"));
        } else {
            this.p1 = new Point(imagePoint.x, imagePoint.y, this.measurementColor, this.fabricCanvas, this.imageIdentifier, this.viewer);
            this.p1.render(zoom);
        }
    ...
    selectMeasurement(measurement) {
        ...
        const evt_select = new CustomEvent("measurement-selected", {
            detail: {measurement},
        });
        window.dispatchEvent(evt_select);
    }
    ...
    deselectMeasurement() {
        ...
        const evt_deselect = new CustomEvent("measurement-deselected");
        window.dispatchEvent(evt_deselect);
    }
    ...
    processOptions(options) {
        ...
        this.measurementColor = options.selectedColor || options.measurementColor || "#000000";
...
class Measurement {
    ...
    render(zoom) {
        ...
        this.textObject = new fabric.Text(text, {
            ...
            fontSize: this.plugin.fontSize / zoom,
            fill: this.color,
        });
...
class Point {
    ...
    constructor(x, y, color, fabricCanvas, imageIdentifier) {
        ...
        this.fabricObject = new fabric.Circle({
            ...
            top: this.y,
            fill: this.color,
            radius: this.plugin.fontSize * 0.4,
        });
    }
    ...
    render(zoom) {
        ...
        const viewerPoint = this.viewer.viewport.imageToViewerElementCoordinates(new OpenSeadragon.Point(this.x, this.y));
        const canvasPoint = this.viewer.viewport.viewerElementToImageCoordinates(viewerPoint);
