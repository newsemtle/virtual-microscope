        ...
        takeScreenshot: function() {
	    	...
	    	var downloadFunction = function(){
				...
				if(viewer.screenshotInstance.showOptions && viewer.scalebarInstance && document.getElementById("screenshotDrawScalebar").checked){
					...
					var ctx = Canvas.getContext('2d');
					// 1. Annotorious SVG layer 추가
					const annoSvg = document.querySelector('.a9s-annotationlayer');
					if (annoSvg) {
						const svgString = new XMLSerializer().serializeToString(annoSvg);
						...
						svgImg.onload = function () {
							...
							// 2. OSDMeasure 캔버스 추가 (있을 경우)
							if (viewer.fabricjsOverlay && viewer.fabricjsOverlay.fabricCanvas) {
								const fabricCanvasEl = viewer.fabricjsOverlay.fabricCanvas.lowerCanvasEl;
							...
							// 4. Navigator 추가
							if (viewer.navigator && viewer.navigator.displayRegion) {
								const navCanvas = viewer.navigator.element.querySelector('canvas');
								if (navCanvas) {
									const navX = viewer.navigator.element.offsetLeft;
									const navY = viewer.navigator.element.offsetTop;
									ctx.drawImage(navCanvas, navX, navY);
									console.log(navX, navY);
								}
							}
						...
						// 주석 없을 경우에도 OSDMeasure와 기타 요소들 저장
						if (viewer.fabricjsOverlay) {
							const fabricCanvasEl = viewer.fabricjsOverlay.fabricCanvas.lowerCanvasEl;
							ctx.drawImage(fabricCanvasEl, 0, 0);
						}
